...
const ioEvents = (io: SocketIO.Server | any) => {
  ...
  io.use(function(socket: SocketIO.Socket | any, next: any) {
    isIoAuthenticated(socket, function (err: string) {
      if (err) {
        return next(new Error('authentication error'));
      } else {
        map[socket.x_user.id] = socket.id;
        return next();
      }
    });
  });
  ...
  io.on(ChatEvent.CONNECT, (socket: SocketIO.Socket | any) => {
    ...
    socket.on(ChatEvent.NEW_MESSAGE, (data: any, cb: any) => {
      if (map[data.userId]) {
        IoHelper.sendToPrivate(io, map[data.userId], 'new_mesage', {message: data.message, time: data.time, fromUser: socket.x_user.id });
        cb({sent: true});
      } else {
        cb();
      }
      _addMessage({from: socket.x_user.id, to: data.userId, message: data.message, seen: false, conversation: socket[Statics.CONVERSATION_ID]});
    })
    //listen on read_mesage
    socket.on(ChatEvent.READ_MESSAGE, (data: {userId: string}) => {
      if (map[data.userId]) {
        _setMessagesToSeen({from: socket.x_user.id, to: data.userId});
        IoHelper.sendToPrivate(io, map[data.userId], 'read_mesage', {fromUser: socket.x_user.id });
      }
      _addConversation({participants: [socket.x_user.id, data.userId]}).then((response: any) => {
        socket[Statics.CONVERSATION_ID] = response && response._id;
      });
    })

    socket.on(ChatEvent.DISCONNECT, () => {
      delete map[socket.x_user.id];
      sendOnineList(io, map);
    });
  });
}
